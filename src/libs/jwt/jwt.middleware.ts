...
export class JwtMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    ...
    if ('token' in req.headers && 'refresh_token' in req.headers && 'email' in req.headers) {
      ...
      try {
        ...
        try {
          const isCheckedEmailToken = await bcrypt.compare(user.refreshToken, refresh_token as string);
      ...
      try {
        ...
        const isCheckedIdToken = await bcrypt.compare(user.refreshToken, refresh_token as string);
